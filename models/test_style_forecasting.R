...
# -------------------------
# Load the style frequencies data
data <- read_csv("data/processed/style_frequencies.csv")
...
wide_data <- data %>%
  select(season, style, percentage) %>%
  pivot_wider(
    names_from = season,
...
print("Top styles by actual Fall25 frequency:")
print(results_df)
...
# Save results
write_csv(results_df, "data/lstm/style_forecasting_test_results.csv")
...
p <- ggplot(results_df, aes(x = actual_fall25, y = predicted_fall25)) +
  ...
  labs(
    title = "Actual vs Predicted Style Frequencies for Fall25",
    x = "Actual Frequency (%)",
...
# Save the plot
ggsave("data/lstm/style_forecasting_test_plot.png", p, width = 10, height = 6) 